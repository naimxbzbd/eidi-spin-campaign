(function() {
  // Prize Segments for Wheel Display (11 segments)
  const SEGMENTS = [
    ...
    { name: "Better Luck Next Time", color: "#8395A7" }
  ];

  // Probability: Better Luck 45%, 10TK 30%, 20TK 15%, 50TK 10%
  function getWeightedBasePrize() {
    const rand = Math.random() * 100;
    if (rand < 45) {
      return { prizeKey: "Better Luck Next Time", segmentIdx: 10, value: null, isJackpot: false };
    } else if (rand < 75) {
  ...
  function getSegmentFromRotation(rotation) {
    const segAngle = (Math.PI * 2) / SEGMENTS.length;
    let rawAngle = (rotation % (Math.PI * 2) + Math.PI * 2) % (Math.PI * 2);
    let pointerAngle = (Math.PI * 1.5);
    let diff = (pointerAngle - rawAngle + 2 * Math.PI) % (2 * Math.PI);
    let idx = Math.floor(diff / segAngle) % SEGMENTS.length;
    return idx;
  }
  ...
  function startSpinWithTarget(prizeKey, isJackpotTshirt, amountValue) {
    ...
    if (targetIdx === -1 && prizeKey === "Better Luck Next Time") targetIdx = 10;
  ...
  function performSpin() {
    ...
    // Hidden T-shirt Jackpot: 1% chance
  ...
  function showResultPopup(prize, isJackpot, amount) {
    ...
    if (isJackpot && prize === "টি-শার্ট") {
      ...
      titleEl.innerHTML = "😔 দুঃখিত!";
      msgEl.innerHTML = "এবার ভাগ্য সহায় হয়নি। পরবর্তী সুযোগে চেষ্টা করুন! 🌙";
    } else {
  ...
  function triggerGoldenGlow() {
    ...
    styleTag.textContent = `@keyframes goldFlash { 0% { box-shadow: 0 0 0 0 gold; border-color: #ffcc44; } 100% { box-shadow: 0 0 0 25px rgba(255,215,0,0); border-color: #ffaa33; } }`;
